...
and a daily panel of TC activity in the Australian region:
    date, tc_active, n_tcs_active, tc_max_wind, tc_names
"""
...
def tc_daily_panel(tracks: pd.DataFrame, start=None, end=None) -> pd.DataFrame:
    ...
    # no TC (or wind not recorded) -> zero severity
    panel["tc_max_wind"] = panel["tc_max_wind"].fillna(0.0)
    panel["tc_names"] = panel["tc_names"].apply(lambda v: v if isinstance(v, list) else [])
    return panel.reset_index()[["date", "tc_active", "n_tcs_active", "tc_max_wind", "tc_names"]]
